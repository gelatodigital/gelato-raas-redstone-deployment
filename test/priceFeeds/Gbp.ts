...
import { PriceFeedAdapterGBP, PriceFeedGBP, SimplePriceFeedConsumer } from "../../typechain";
import { Contract } from "ethers";
import { SignerWithAddress } from "@nomiclabs/hardhat-ethers/signers";
import hre from "hardhat";
const getLatestSignedGBPPrice = async () => {
...
describe("ClassicModelGBP", function () {
  let priceFeedAdapter: PriceFeedAdapterGBP;
  let priceFeed: PriceFeedGBP;
  ...
  before(async () => {
    ...
    priceFeedAddress = (await deployments.get("PriceFeedGBP")).address;
    ...
    priceFeedAdapterAddress = (await deployments.get("PriceFeedAdapterGBP")).address;
    priceFeedAdapter = (await ethers.getContractAt(
      "PriceFeedAdapterGBP",
      priceFeedAdapterAddress
    )) as PriceFeedAdapterGBP;
  ...
  it("Should do a simple independent update", async () => {
    ...
    const dataPackagesResponse = await getLatestSignedGBPPrice();
    const wrappedAdapter =
      WrapperBuilder.wrap(priceFeedAdapter).usingDataPackages(
        dataPackagesResponse
      );
    const { dataPackage } = dataPackagesResponse["GBP"]![0];

    const parsedPrice = parsePrice(dataPackage.dataPoints[0].value);
    console.log(`Setting GBP price in PriceFeed contract to: ${parsedPrice}`);
  ...
  it("Should read latest price data", async () => {
    ...
    console.log(`Latest value the GBP Price Feed: ${latestRoundData.answer}`);
  });
  ...
  it("Should update price and read it in a single transaction", async () => {
    ...
    const { dataPackage } = dataPackagesResponse["GBP"]![0];
    const parsedPrice = parsePrice(dataPackage.dataPoints[0].value);
    console.log(`User sees in the UI GBP price: ${parsedPrice}`);
