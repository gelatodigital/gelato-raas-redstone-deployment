...
import {  SimplePriceFeedConsumer, } from "../../typechain";
import { Contract } from "ethers";
import { SignerWithAddress } from "@nomiclabs/hardhat-ethers/signers";
import hre from "hardhat";
import { PriceFeedAdapterMock, PriceFeedMock } from "../../typechain/contracts/PriceFeeds/MOCK";
const getLatestSignedMOCKPrice = async () => {
...
describe("ClassicModelMock", function () {
  ...
  before(async () => {
   ...
   await deployments.fixture()
    ...
    priceFeedAddress = (await deployments.get("PriceFeedMock")).address;
    ...
    priceFeedAdapterAddress = (await deployments.get("PriceFeedAdapterMock")).address;
    priceFeedAdapter = (await ethers.getContractAt(
      "PriceFeedAdapterMock",
      priceFeedAdapterAddress
    )) as PriceFeedAdapterMock;
  ...
  it("Should do a simple independent update", async () => {
    ...
    const dataPackagesResponse = await getLatestSignedMOCKPrice();
    const wrappedAdapter =
      WrapperBuilder.wrap(priceFeedAdapter).usingDataPackages(
        dataPackagesResponse
      );
    const { dataPackage } = dataPackagesResponse["MOCK"]![0];

    const parsedPrice = parsePrice(dataPackage.dataPoints[0].value);
    console.log(`Setting MOCK price in PriceFeed contract to: ${parsedPrice}`);
  ...
  it("Should read latest price data", async () => {
    ...
    console.log(`Latest value the MOCK Price Feed: ${latestRoundData.answer}`);
  });
  ...
  it("Should update price and read it in a single transaction", async () => {
    ...
    const { dataPackage } = dataPackagesResponse["MOCK"]![0];
    const parsedPrice = parsePrice(dataPackage.dataPoints[0].value);
    console.log(`User sees in the UI MOCK price: ${parsedPrice}`);
